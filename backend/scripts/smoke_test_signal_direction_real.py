...
def _run(asset_type: str, symbol: str, days: int, gap: int, chunk_delay: float) -> None:
    ...
    for name, values, inverse in (
        ...
        print(f"{name} direct:   ROC-AUC={direct_roc if direct_roc is not None else float('nan'):.4f}, PR-AUC={direct_pr:.4f}, Spearman={direct_spear:.4f}")
        print(f"{name} inverse:  ROC-AUC={inverse_roc if inverse_roc is not None else float('nan'):.4f}, PR-AUC={inverse_pr:.4f}, Spearman={inverse_spear:.4f}")
