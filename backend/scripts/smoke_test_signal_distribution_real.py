...
def _run(asset_type: str, symbol: str, days: int, gap: int, chunk_delay: float) -> None:
    ...
    print(f"Signal score distribution: {_describe(oos['signal_score'])}")
    ...
    for fold, group in oos.groupby("fold", sort=True):
        print(
            f"  Fold {fold}: signal {_describe(group['signal_score'])} | "
            f"ML {_describe(group['ml_probability'])} | "
            f"technical {_describe(group['technical_score'])} | "
            f"risk {_describe(group['risk_adjustment'])} | "
            f"target={group['target'].mean():.3f}"
        )
    print("REAL SIGNAL DISTRIBUTION SMOKE TEST PASSED")
